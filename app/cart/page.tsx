      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        {!loaded ? null : items.length === 0 ? (
          <div className="text-center py-24">
            ...
            <a href="/shop" className="bg-zinc-900 text-white font-bold px-8 py-3 rounded-xl hover:bg-gray-50 transition-all">
              Browse Shop →
            </a>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 flex flex-col gap-4">
              {items.map((item) => (
                <div key={item.sku} className="bg-gray-50 rounded-2xl border border-gray-200 shadow-sm p-5 flex gap-5 items-start">
                  <img
                    src={item.image}
                    alt={item.name}
                    className="w-20 h-20 object-contain rounded-xl bg-gray-50 border border-gray-200 flex-shrink-0"
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-zinc-500 font-mono mb-1">{item.sku}</p>
                    ...
                    <div className="flex items-center gap-4 flex-wrap">
                      ...
                      <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                        <button
                          ...
                          className="px-3 py-1.5 text-zinc-500 hover:bg-gray-50 transition-colors font-bold text-lg leading-none"
                        >−</button>
                        <span className="px-4 py-1.5 text-sm font-semibold text-zinc-900 border-x border-gray-200">{item.qty}</span>
                        <button
                          onClick={() => updateQty(item.sku, item.qty + 1)}
                          className="px-3 py-1.5 text-zinc-500 hover:bg-gray-50 transition-colors font-bold text-lg leading-none"
                        >+</button>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    {item.qty > 1 && (
                      <p className="text-xs text-zinc-500">${item.price.toLocaleString()} each</p>
                    )}
                  </div>
                </div>
              ))}
              ...
              <button
                onClick={clearCart}
                className="text-xs text-zinc-500 hover:text-red-500 transition-colors self-start mt-2"
              >
                Clear cart
              </button>
            </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-gray-50 rounded-2xl border border-gray-200 shadow-sm p-6 sticky top-24">
                ...
                <div className="flex flex-col gap-3 text-sm mb-5">
                  ...
                  {shipping > 0 && (
                    <p className="text-xs text-zinc-500">Free shipping on orders $500+</p>
                  )}
                  <div className="border-t border-gray-200 pt-3 flex justify-between font-black text-zinc-900 text-base">
                    <span>Total</span>
                ...
                <a
                  href="/checkout"
                  className="block w-full text-center bg-zinc-900 text-white font-bold text-sm py-3.5 rounded-xl hover:bg-gray-50 transition-all mb-3"
                >
                  Proceed to Checkout →
                </a>
                <a
                  href="/shop"
                  className="block w-full text-center border border-gray-200 text-zinc-500 font-semibold text-sm py-3 rounded-xl hover:bg-gray-50 transition-all"
                >
                  Continue Shopping
                </a>
