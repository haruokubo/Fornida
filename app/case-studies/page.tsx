...
const TAG_COLORS: Record<string, string> = {
  Healthcare: "text-zinc-300 bg-zinc-800 border-zinc-700",
  "AI Automation": "text-zinc-300 bg-zinc-800 border-zinc-700",
...
export default function CaseStudiesPage() {
  return (
    <main className="min-h-screen bg-zinc-950 text-white">
      ...
      {/* NAV */}
      <nav className="sticky top-0 z-50 bg-zinc-950/90 backdrop-blur-md border-b border-zinc-800 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <a href="/case-studies" className="text-sm text-white font-semibold border-b border-white pb-0.5">Case Studies</a>
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="flex flex-col gap-8">
          {CASE_STUDIES.map((cs, i) => (
            <article
              key={cs.id}
              className="group bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden hover:border-zinc-700 transition-all duration-300"
            >
              <div className="p-8 md:p-10">
                <div className="flex flex-wrap items-center gap-3 mb-5">
                  <span className={`text-xs font-bold px-3 py-1 rounded-full border ${TAG_COLORS[cs.tag] ?? "text-zinc-300 bg-zinc-800 border-zinc-700"}`}>
                    {cs.tag}
                  </span>
                  {cs.client && (
                    <span className="text-xs text-zinc-500 font-mono">{cs.client}</span>
                  )}
                  <span className="text-xs text-zinc-600 ml-auto">{cs.date}</span>
                </div>

                <h2 className="text-2xl md:text-3xl font-black tracking-tight text-white mb-4 leading-snug group-hover:text-zinc-100 transition-colors">
                  {cs.title}
                </h2>
                ...
                <div className="flex flex-wrap gap-6 pt-6 border-t border-zinc-800">
                  ...
                  <div className="ml-auto self-center">
                    <span className="text-sm text-zinc-400 group-hover:text-white transition-colors font-semibold">
                      Read case study →
                    </span>
                  </div>
                </div>
      ...
      {/* CTA */}
      <section className="border-t border-zinc-800 py-20 px-6 text-center bg-zinc-900">
        <h2 className="text-3xl font-black text-white mb-4">Want results like these?</h2>
        <p className="text-zinc-400 mb-8 max-w-xl mx-auto">One free assessment. We'll find your biggest quick win — on us.</p>
        <a href="/#assessment" className="bg-white text-zinc-950 font-bold px-8 py-4 rounded-xl hover:bg-zinc-100 transition-all">
